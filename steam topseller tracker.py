...
# 붉은사막 실적 기반 가중치 (best.html STS_W와 동기화)
# Sensor Tower 500만 실측 + 1Q26 NA/EU 81% 매출 비중 반영
# NA+EU ~80%, 중국 매출 기여 낮음(위안화 단가), 일본 Steam 실측 밖
STEAM_WEIGHTS = {
    # 북미 (~30%)
    "us":  22.0,
    "ca":   6.0,
    ...
    "tr":   1.0,   # 리라 단가 낮음
    # 아시아-태평양 (~18%)
    "cn":   5.0,   # Steam 구매자↑ but 위안화 단가→매출 기여 하향
    "kr":   3.5,   # 개발사 본국, 팬덤 강함
    "au":   3.0,
    "jp":   1.5,   # Steam 실측 상위권 밖, 장르 특성 하향
    "tw":   1.0,
    "hk":   0.8,
    "sg":   0.8,
    "nz":   0.5,
